...
class NestedDefaultDict(defaultdict):
    def __init__(self,*args):
        super(NestedDefaultDict, self).__init__(*(args or (int,)))
    ...
    def merge(self, other):
        ...
        for k, v in other.items():
            if (k in self and isinstance(self[k], dict) and isinstance(other[k], collectionsAbc.Mapping)):
                merge(self[k], other[k])
            else:
                self[k] = other[k]
